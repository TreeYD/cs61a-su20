    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if isinstance(place.ant, ContainerAnt):
                assert place.ant.can_contain(self) , 'Two ants in {0}'.format(place)
                place.ant.contain_ant(self)
            if isinstance(self, ContainerAnt):
                assert self.can_contain(place.ant), 'Two ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while target_place is not beehive:
            if dist >= self.min_range and target_place.bees:
                return rANTdom_else_none(target_place.bees)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        copy_bees = list(cur_place.bees)
        reflect_damage = amount + (self.damage if self.armor <= 0 else 0) 
        for bee in copy_bees:
            bee.reduce_armor(reflect_damage)
        # END Problem 5
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        while double_place:
            ant = double_place.ant 
            if ant and ant not in self.double_list:
                ant.damage *= 2
                self.double_list.append(ant)
            if isinstance(ant, ContainerAnt):
                c_ant = ant.contained_ant
                if c_ant and c_ant not in self.double_list:
                    self.double_list.append(c_ant)
                    c_ant.damage *= 2
    ...
    def reduce_armor(self, amount):
        ...
        if self is QueenAnt.true_queen and self.armor <= 0:
            QueenAnt.true_queen = None
            bees_win()
        # END Problem 13
    def remove_from(self, place):
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        if self.direction == -1:
            if not isinstance(self.place, Hive):
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        return max(self.damage - distance*0.2 - self.insects_shot*0.05, 0)
        # END Problem OPTIONAL
