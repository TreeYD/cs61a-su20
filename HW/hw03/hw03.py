...
def g_iter(n):
    ...
    index = 4
    while index <= n:
        G[index%3] = G[(index - 1)%3] + G[(index - 2)%3] * 2 + G[(index - 3)%3] * 3 
        index = index + 1
    return G[n%3]
...
def missing_digits(n):
    ...
    ten = n % 100 // 10
    mis = one - ten - (ten != one)
    return missing_digits(n//10) + mis 
...
def move_stack(n, start, end):
    ...
    if n == 0:
        return 
    idle = 6 - start - end # get the index of idling rod
    move_stack(n-1, start, idle)
    print_move(start, end)
    move_stack(n-1, idle, end)
    return 
...
def make_anonymous_factorial():
    ...
    True
    """
    return (lambda F:lambda n:F(F, n))(lambda fact,n:1 if n == 1 else mul(n, fact(fact, sub(n, 1))))
