...
def subseqs(s):
    ...
    if not s:
        ...
        subres = subseqs(s[1:])
        return subres + insert_into_all(s[0], subres)
...
def reverse(lst):
    ...
    while id < list_len/2:
        tmp = lst[id]
        lst[id] = lst[list_len - id - 1]
        lst[list_len - id - 1] = tmp
        id += 1
    return
cs61a = {
    "Homework": 2,
...
def make_glookup(class_assignments):
    ...
    full = 0
    earn = 0
    def calc(assignment, point):
        nonlocal full, earn
        earn += point
        full += class_assignments[assignment]
        return earn / full
    return calc
...
def make_advanced_counter_maker():
    ...
    def make_counter():
        person_num = 0
        def counter(message):
            nonlocal person_num, global_num
            "*** YOUR CODE HERE ***"
