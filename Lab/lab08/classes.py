...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        player_new_cards = []
        for player_old_card in player.deck.cards:
            player_new_card = player_old_card.copy()
            player_new_card.attack += other_card.attack
            player_new_card.defense += other_card.defense
            player_new_cards += [player_new_card]
        player.deck = Deck(player_new_cards)
        
        opponent_new_cards = []
        for opponent_old_card in opponent.deck.cards:
            if opponent_old_card.attack != other_card.attack and opponent_old_card.defense != other_card.defense:
                opponent_new_card = opponent_old_card.copy()
                opponent_new_cards += [opponent_new_card]
        opponent.deck = Deck(opponent_new_cards)
